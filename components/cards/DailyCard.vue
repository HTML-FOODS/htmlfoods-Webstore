  ...
  <div>
    <a-row type="flex" style="margin-bottom: 1rem">
      <a-col :lg="{ span: 24 }" :xs="{ span: 24 }">
        <div class="title-head">{{  categoryName  }} </div>
      </a-col>
    </a-row>

    <a-row type="flex" justify="start" v-if="products.length >= 1">
      <a-col :lg="{ span: 6 }" :sm="{ span: 22 }" :xs="{ span: 24 }" v-for="(product, index) in products" :key="index">
    ...
    <a-modal v-model="modalVisible" centered width="350px" @ok="() => (modalVisible = false)" :footer="null"
      class="card-modal">
      <a-row>
        <a-col :span="24">
